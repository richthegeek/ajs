...
(function() {
  ...
  module.exports = Evaluator = (function() {
    ...
    Evaluator.prototype.set = function(key, target, value, callback) {
      var fn;
      if ((target != null ? target.__set : void 0) != null) {
        fn = target.__set.bind(target);
      } else {
        fn = function(key, value, callback) {
          target[key] = value;
          return callback(null, value);
        };
      }
      return fn(key, value, callback);
    };

    Evaluator.prototype.unset = function(key, target, callback) {
      var fn;
      if ((target != null ? target.__unset : void 0) != null) {
        fn = target.__unset.bind(target);
      } else {
        fn = function(key, target, callback) {
          delete target[key];
          return callback();
        };
      }
      return fn(key, target, callback);
    };
